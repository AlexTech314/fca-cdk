...
function NewsGridPreview({ posts }: { posts: BlogPost[] }) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {posts.map((post) => (
        <button
          key={post.id}
          onClick={() => requestNavigation(`/admin/news/${post.slug}`)}
        ...
        >
          <p className="text-sm text-secondary">{formatDate(post.publishedAt)}</p>
...
function NewsPageContent({ posts }: { posts: BlogPost[] }) {
    ...
    <div className="bg-background">
      ...
      <section className="py-12 md:py-16">
        <div className="container-max">
          ...
          {viewMode === 'grid' ? (
            <NewsGridPreview posts={posts} />
          ) : (
            <BlogPostsTable initialPosts={posts} category="news" detailBasePath="/admin/news" />
          )}
        </div>
      </section>
...
export default function AdminNewsPage() {
  ...
  return (
    <AdminPageProvider pageKey="news" initialData={pageData}>
      <NewsPageContent posts={posts} />
      <SaveBar />
    </AdminPageProvider>
  );
}
