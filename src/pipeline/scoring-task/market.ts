...
export async function refreshLeadRanks(db: PrismaClient): Promise<void> {
  ...
  const excluded = await db.$executeRaw`
    UPDATE leads SET composite_score = 0, tier = NULL
    WHERE is_excluded = true AND (composite_score IS NULL OR composite_score != 0)`;

  console.log(`Lead priority scores refreshed: ${result} rows updated, ${excluded} excluded zeroed`);
}
